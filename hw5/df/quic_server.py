...
class QUICServer:
    def __init__(self):
        ...
        self.server_congestion_window_size = 32
        self.server_flow_control_window_size = 10000
    ...
    def connect(self, client_addr):
        ...
        self.server_socket.connect(self.client_addr)
    
    def accept(self):
        """this method is to indicate that the client can connect to the server now"""
        # 第二次握手
        while True:
            recv_packet, client_addr = self.server_socket.recvfrom(1024)
            ...
            if recv_setup == 1 and recv_syn == 1 and recv_ack == 0:
                self.connect(client_addr)
                break
        recv_flow_control_window_size = int.from_bytes(recv_payload, byteorder='big')
        self.server_flow_control_window_size = max(recv_flow_control_window_size, self.server_flow_control_window_size)
        sent_setup, sent_syn, sent_ack, sent_stream_id, sent_last_num, sent_now_num = 1, 1, 1, 0, 0, 0
        sent_hdr = struct.pack('!BBBBBB', sent_setup, sent_syn, sent_ack, sent_stream_id, sent_last_num, sent_now_num)
        sent_payload = self.server_flow_control_window_size.to_bytes(2, byteorder='big')
        sent_packet = sent_hdr + sent_payload
        
        timeout = self.server_socket.gettimeout()
        self.server_socket.settimeout(ack_time_out)
        while True:
            try:
                self.server_socket.send(sent_packet)
                recv_packet = self.server_socket.recv(1024)
                recv_hdr = recv_packet[:6]
                recv_payload = recv_packet[6:]
                recv_setup, recv_syn, recv_ack, recv_stream_id, recv_last_num, recv_now_num = struct.unpack('!BBBBBB',recv_hdr)
                # print(recv_setup, recv_syn, recv_ack, recv_stream_id, recv_last_num, recv_now_num)
                if recv_setup == 1 and recv_syn == 0 and recv_ack == 1:
                    break
    ...
    def send(self, stream_id: int, data: bytes):
        ...
        unit = buffer_unit(stream_id=stream_id, window_size=self.server_congestion_window_size, flow_window=self.server_flow_control_window_size)
        unit.sent_init(data)
    ...
    def close(self):
        ...
        sent_hdr = struct.pack('!BBBBBB', sent_setup, sent_syn, sent_ack, sent_stream_id, sent_last_num, sent_now_num)
        self.server_socket.send(sent_hdr)
